...
<li class="list-inline-item text-capitalize m-2">{{$t('Page')+': '+current_page+' / '+last_page}}</li>
    ...
    <tr v-for="item in items" :key="item.keydb23">
        ...
        <td>
        <router-link type="link" class="btn-sm btn-primary m-1" :title="$t('edit')" :to="{ name: 'Edit Category',params: { id:item.id }  }">
         Edit
        </router-link>
...
data() {
        ...
        current_page:null,
        last_page:null ,
        path:this.$store.state.backendUrl+"/admin/category",
        first_page_url:"",
        last_page_url:"",
        next_page_url:"",
        prev_page_url:"",
            ...
            .then((response) => {
                ...
                if(!this.isSearch){this.roles = response.data.roles}
                  ...
                  console.log(this.items)
                    ...
                    this.path=this.$store.state.backendUrl+"/admin/category";
