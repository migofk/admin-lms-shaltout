...
<CRow>
 ...
 <!-- input -->
 <CCol :md="12"> 
  <CFormLabel for="AddAbout_description">{{ $t('Decription') }}*</CFormLabel>
...
methods: {
    ...
    getCreateData(){
        ...
        axios
        .get(this.$store.state.backendUrl+'/admin/about_edit'+query  , {
            headers: {
    ...
    getTranslation(lang_id){
        ...
        if(toEdit){
            ...
            this.$router.push({ name: 'About', query: { lang_id: lang_id,id:editId,ref_id:this.ref_id }});
        }
    },
    onSubmit(event) {
        ...
        axios
            .post(
            this.$store.state.backendUrl + '/admin/about_update/'+this.main_id,
            formData,
            { headers: { 
                'Authorization': 'Bearer ' + this.$store.state.token, 
